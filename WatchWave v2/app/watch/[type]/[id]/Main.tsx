...
const Main = ({ params }: { params: { type: string; id: number } }) => {
  ...
  useEffect(() => {
    ...
    if (value.continueWatching && value.continueWatching.length > 0) {
      ...
      // set isInCW to true
      if (
        value.continueWatching.some(
          (item: { id: number }) => item.id === result.id,
        )
      ) {
  ...
  useEffect(() => {
    if (type === "movie") {
      fetch(
        ...
        .then((data) => {
          fetch(
            `https://api.themoviedb.org/3/${type}/${id}/release_dates?language=en-US`,
            options,
          )
            .then((res2) => res2.json())
            .then((data2) => {
              ...
              for (let i = 0; i < data2.results.length; i++) {
                if (countries.includes(data2.results[i].iso_3166_1)) {
                  content_rating =
                    data2.results[i].release_dates[0].certification;
                  break;
                }
              }
      ...
      fetch(
        ...
        .then((data) => {
          fetch(
            `https://api.themoviedb.org/3/tv/${id}/content_ratings?language=en-US`,
            options,
          )
            .then((res2) => res2.json())
            .then((data2) => {
              ...
              Promise.all(data.seasons.map(fetchSeasonData)).then(
                (tempContent: Episode[][]) => {
        ...
        <Modal isOpen={adb.isOpen} onOpenChange={adb.onOpenChange}>
          <ModalContent>
            <>
              ...
              <ModalBody>
                <p>
                  It is <b>highly</b> recommended that you use an ad blocker
                  such as{" "}
                  <NLink
                    as={Link}
                    isExternal
                  ...
                  while watching videos on WatchWave. There are spammy ads on
                  the video players, which are out of our control and can&apos;t
                  be disabled. Using an ad blocker is an easy solution to get
                  rid of the ads on the video players.
                </p>
                <br />
              </ModalBody>
              <ModalFooter>
